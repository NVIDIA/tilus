...
from tilus.hidet.ir.expr import Expr, as_expr, logical_and


def index_serialize(
    indices: Sequence[Union[Expr, int]], shape: Sequence[Union[Expr, int]], ranks: Optional[Sequence[int]] = None
) -> Expr:
    """
...
def index_deserialize(
    scalar_index: Union[Expr, int], shape: Sequence[Union[Expr, int]], ranks: Optional[Sequence[int]] = None
) -> List[Expr]:
...
def index_within_bound(
    indices: Sequence[Union[Expr, int]],
    lower_bound: Union[Sequence[Union[Expr, int]], Expr, int],
    upper_bound: Union[Sequence[Union[Expr, int]], Expr, int],
) -> Expr:
...
def vector_mul(lhs: Sequence[int], rhs: Union[Sequence[int], int]) -> List[int]:
...
def vector_add(lhs: Sequence[int], rhs: Union[Sequence[int], int]) -> List[int]:
    if isinstance(rhs, int):
        rhs = [rhs for _ in lhs]
    assert len(lhs) == len(rhs), "Expect both indices have the same length"
    return [a + b for a, b in zip(lhs, rhs)]
...
def vector_sub(lhs: Sequence[int], rhs: Union[Sequence[int], int]) -> List[int]:
    if isinstance(rhs, int):
        rhs = [rhs for _ in lhs]
    assert len(lhs) == len(rhs), "Expect both indices have the same length"
    return [a - b for a, b in zip(lhs, rhs)]
...
def vector_div(lhs: Sequence[int], rhs: Union[Sequence[int], int]) -> List[int]:
    if isinstance(rhs, int):
        rhs = [rhs for _ in lhs]
    assert len(lhs) == len(rhs), "Expect both indices have the same length"
    return [a // b for a, b in zip(lhs, rhs)]
...
def vector_within_bound(
    ...
    lower_bound: Union[Sequence[int], int],
    upper_bound: Union[Sequence[int], int],
) -> bool:
    # check if the indices are within the bound
