...
class StmtRewriter(StmtFunctor, BaseRewriter):
    ...
    def visit_dispatch(self, node: Node):
        ...
        if self._prologues:
            if isinstance(ret, SeqStmt):
                ...
                ret = seq_stmt(self._prologues + [ret])
            self._prologues = []
        return ret
    ...
    def visit_BufferStoreStmt(self, stmt: BufferStoreStmt):
        ...
        if same_node(buf, stmt.buf) and same_list(indices, stmt.indices) and same_node(value, stmt.value):
            ...
            return buffer_store_stmt(buf, indices, value, stmt.protected)
    ...
    def visit_LetStmt(self, stmt: LetStmt):
        ...
        if (
            same_list(bind_vars, stmt.bind_vars)
            and same_list(bind_values, stmt.bind_values)
            and same_node(body, stmt.body)
        ):
            return stmt
        else:
    ...
    def visit_AssertStmt(self, stmt: AssertStmt):
        ...
        if same_node(cond, stmt.cond):
            ...
            return assert_stmt(cond, stmt.msg)
    ...
    def visit_LaunchKernelStmt(self, stmt: LaunchKernelStmt):
        ...
        if same_list(
            ...
            return launch_kernel_stmt(func_var, args, grid_dim, cluster_dim, block_dim, shared_mem_bytes, stmt.target)
    ...
    def visit_BlackBoxStmt(self, stmt: BlackBoxStmt):
        ...
        if same_list(exprs, stmt.exprs):
            ...
            return black_box_stmt(stmt.template_string, *exprs)
