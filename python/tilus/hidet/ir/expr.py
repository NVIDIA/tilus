...
class Expr(Node):
    ...
    def write(self, items, value, protected=True):
        from tilus.hidet.ir.stmt import BufferStoreStmt  # noqa: PLC0415, buffer_store_stmt

        te = self[items]
        if not isinstance(te, TensorElement):
            raise ValueError("expect element indexing, but got slicing.")
        return buffer_store_stmt(buf=self, indices=te.indices, value=value, protected=protected)
...
def as_expr(obj: Union[float, bool, int, str, Expr]) -> Expr:
    """Lift a Python scalar to an ``Expr``.

    Ints pick the narrowest standard integer dtype that fits: ``int32`` by
    default, widening to ``uint32`` / ``int64`` / ``uint64`` for bit patterns
    that don't. Callers who need a specific dtype should wrap explicitly
    (``uint32(x)``, ``int64(x)``, ...) instead of relying on this default.
    """
    from tilus.hidet.ir.dtypes import int64, uint32, uint64  # noqa: PLC0415
    ...
    if isinstance(obj, int):
        if default_int_dtype.imin <= obj <= default_int_dtype.imax:
            return default_int_dtype.constant(obj)
        if 0 <= obj <= uint32.imax:
            return uint32.constant(obj)
        if int64.imin <= obj <= int64.imax:
            return int64.constant(obj)
        if 0 <= obj <= uint64.imax:
            return uint64.constant(obj)
        raise ValueError(f"Integer {obj} does not fit in any standard integer dtype.")
...
def tensor_element(base: Expr, indices: Sequence[Int], protected: bool = False) -> TensorElement:
    return TensorElement(base=base, indices=tuple(as_expr(i) for i in indices), protected=protected)


def _chain_binary_op(op: Type[BinaryExpr], operands, default):
    if len(operands) == 0:
        return as_expr(default)
    if len(operands) == 1:
        return as_expr(operands[0])
    a = _chain_binary_op(op, operands[:-1], default)
    b = as_expr(operands[-1])
...
def logical_not(a: Union[Expr, PyScalar]):
    return _unary(LogicalNot, as_expr(a))


def bitwise_not(a: Union[Expr, PyScalar]):
    return _unary(BitwiseNot, as_expr(a))
...
def call(func: Var, args: Sequence[Union[Expr, PyScalar]]) -> Call:
    return Call(func_var=func, args=tuple(as_expr(a) for a in args))
...
def view(ptr: Expr, tp: TensorType) -> Expr:
    ...
    return cast(ptr, tensor_pointer_type_from(tp))
