...
from tilus.hidet.ir.stmt import AssignStmt, DeclareStmt, asm, asm_stmt, assign_stmt, declare_stmt
...
def register_mma_instructions():
    for config in mma_configs.values():
        ...
        with FunctionBuilder(name=func_name, kind="cuda_internal") as fb:
            ...
            fb += assign_stmt(ra, cast(a, ra.type))
            fb += assign_stmt(rb, cast(b, rb.type))
            fb += assign_stmt(rc, cast(c, rc.type))
            fb += asm_stmt(
