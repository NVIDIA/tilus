...
def register_cvta_instructions():
    from tilus.hidet.ir.type import void_p as void_p_type
    from tilus.hidet.lang import attrs, u32
    ...
    # Extended cvta instructions (from extensions)
    for src_space in ["generic"]:
        for dst_space in ["cta_shared", "cluster_shared"]:
            ...
            register_primitive_function(name=cvta_ext.name, func_or_type=cvta_ext)
    ...
    for src_space in ["cta_shared", "cluster_shared"]:
        for dst_space in ["generic"]:
            ...
            @script
            def cvta_ext(src: u32) -> void_p_type:
                attrs.func_name = func_name
                attrs.func_kind = "cuda_internal"
                ret: void_p_type = 0
                asm(
...
def cvta_generic_to_cta_shared(generic_addr: Expr) -> Expr:
