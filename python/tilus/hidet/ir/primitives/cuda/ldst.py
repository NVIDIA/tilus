...
from tilus.hidet.ir.type import DataType, PointerType, TensorPointerType, data_type, pointer_type
    ...
    for load_bytes in [4, 8, 16, 32]:
        for cache_operation in ["always", "last_use"]:
            ...
            with FunctionBuilder(func_name, kind="cuda_internal") as fb:
                ...
                body = asm_stmt(
                    doc,
                    outputs=[("=r", deref(cast(var, pointer_type(u32)))) for var in reg_vars],
                    inputs=[("l", addr_var), ("r", cast(pred_var, i32))]
                    + [("r", deref(cast(var, pointer_type(u32)))) for var in reg_vars]
                    + extra_addr_var,
                    is_volatile=True,
                )
                fb.set_body(body)
...
def register_lds():
    ...
    for load_bytes in [4, 8, 16]:
        ...
        with FunctionBuilder(func_name, kind="cuda_internal") as fb:
            ...
            body = asm_stmt(
                doc,
                outputs=[(f"={indicator}", deref(cast(var, pointer_type(dtype)))) for var in reg_vars],
                inputs=[("l", addr_var)],
                is_volatile=True,
            )
            fb.set_body(body)
    ...
    for store_bytes in [1, 4, 8, 16]:
        ...
        with FunctionBuilder(func_name, kind="cuda_internal") as fb:
            ...
            body = asm_stmt(
                doc,
                outputs=[],
                inputs=[("l", addr_var)] + [(indicator, deref(cast(var, pointer_type(dtype)))) for var in reg_vars],
                is_volatile=True,
            )
            fb.set_body(body)
        register_primitive_function(name=func_name, func_or_type=fb.get())
