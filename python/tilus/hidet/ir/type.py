...
class TensorType(BaseType):
    ...
    dtype: DataType
    shape: Array[tvm_ffi.Object]  # Expr — kept as Object to avoid a circular import
...
class PointerType(BaseType):
    ...
    base_type: BaseType
    specifiers: Array[str] = field(default=())
...
class OpaqueType(BaseType):
    ...
    cpp_name: str
    modifiers: Array[str] = field(default=())
...
def tensor_type(dtype, shape: Sequence[Union[int, "Expr"]]) -> TensorType:
    ...
    assert is_seq(shape)
    shape_tuple = tuple(as_expr(v) for v in shape)
    return TensorType(dtype=dtype, shape=shape_tuple)


def pointer_type(
    base_type: Union[str, BaseType],
    specifiers: Sequence[str] = (),
    use_bracket: bool = False,
) -> PointerType:
    return PointerType.create(base_type=base_type, specifiers=specifiers, use_bracket=use_bracket)
...
def tensor_pointer_type_from(tp: TensorType) -> TensorPointerType:
    return TensorPointerType(tensor_type=tp)
...
def func_type(param_types: Sequence[BaseType], ret_type: BaseType) -> FuncType:
    return FuncType(param_types=tuple(param_types), ret_type=ret_type)


def opaque_type(cpp_name: str, *modifiers: str) -> OpaqueType:
    return OpaqueType(cpp_name=cpp_name, modifiers=tuple(modifiers))
