...
# (``Expr``, ``Var``, ``Stmt``, ``Sequence[Stmt]``, ...) — no scalar-to-Expr
# coercion happens inside ``create``. Callers that start from raw Python
# values must lift them to ``Expr`` first via ``as_expr`` (scalar → Expr)
# or a typed constructor like ``int32(...)`` / ``uint32(...)`` / ``boolean(...)``
# (the stmt_builder / transpiler / other IR-constructing layers own that
# coercion).
#
...
class BufferStoreStmt(Stmt):
    ...
    @classmethod
    def create(cls, buf: Expr, indices: Sequence[Expr], value: Expr, protected: bool = False) -> BufferStoreStmt:
...
class ForStmt(Stmt):
    ...
    @classmethod
    def create(cls, loop_var: Var, extent: Expr, body: Stmt, *, attr: Optional[ForStmtAttr] = None) -> ForStmt:
        return cls(
            loop_var=loop_var,
            extent=extent,
            body=body,
...
def asm(
    ...
    *,
    ...
    for output in outputs:
        ...
        updated_outputs.append((constraint, as_expr(output)))
    for output_input in output_inputs:
        constraint = "+" + get_register_type(output_input)
        updated_outputs.append((constraint, as_expr(output_input)))
    for x in inputs:
        constraint = get_register_type(x)
        updated_inputs.append((constraint, as_expr(x)))
...
def launch_kernel(
    ...
    for dims in [grid_dim, cluster_dim, block_dim]:
        ...
        while len(dims) < 3:
            dims.append(1)
        launch_config.append(tuple(as_expr(d) for d in dims))
    ...
    return launch_kernel_stmt(
        ...
        shared_mem_bytes=as_expr(shared_mem),
        target=target,
    )


# ---------------------------------------------------------------------------
# Snake_case factory helpers — these are the user-facing constructors.
# ``XxxStmt.create(...)`` is an internal implementation detail; consumer code
# should go through the factories below so we can tighten argument handling
# or swap out the class implementation without churning every call site.
# ---------------------------------------------------------------------------


def evaluate_stmt(expr: Expr) -> EvaluateStmt:
    return EvaluateStmt.create(expr)
...
def declare_stmt(
    ...
    return DeclareStmt.create(var, init=init, is_static=is_static, scope=scope)


def buffer_store_stmt(buf: Expr, indices: Sequence[Expr], value: Expr, protected: bool = False) -> BufferStoreStmt:
    return BufferStoreStmt.create(buf, indices, value, protected=protected)


def assign_stmt(var: Var, value: Expr) -> AssignStmt:
    return AssignStmt.create(var, value)


def return_stmt(ret_value: Optional[Expr] = None) -> ReturnStmt:
    return ReturnStmt(ret_value=ret_value)
...
def for_stmt(loop_var: Var, extent: Expr, body: Stmt, *, attr: Optional[ForStmtAttr] = None) -> ForStmt:
    return ForStmt.create(loop_var, extent, body, attr=attr)


def while_stmt(cond: Expr, body: Stmt) -> WhileStmt:
    return WhileStmt.create(cond, body)


def break_stmt() -> BreakStmt:
    return BreakStmt()


def continue_stmt() -> ContinueStmt:
    return ContinueStmt()


def if_stmt(cond: Expr, then_body: Stmt, else_body: Optional[Stmt] = None) -> IfStmt:
    return IfStmt.create(cond, then_body, else_body)


def assert_stmt(cond: Expr, msg: Optional[str] = None) -> AssertStmt:
    return AssertStmt.create(cond, msg)
...
def black_box_stmt(template_string: str, *exprs: Expr) -> BlackBoxStmt:
    return BlackBoxStmt.create(template_string, *exprs)


def seq_stmt(seq: Sequence[Stmt]) -> SeqStmt:
    return SeqStmt.create(seq)


def launch_kernel_stmt(
    func_var: Var,
    args: Sequence[Expr],
    grid_dim: Sequence[Expr],
    cluster_dim: Sequence[Expr],
