...
try:
    from tilus.hidet.ir.compute import (
        ArgReduceCompute,
        GridCompute,
        ReduceCompute,
        ScalarInput,
    ...
    TensorNode = GridCompute = ArgReduceCompute = ReduceCompute = TensorInput = ScalarInput = None
from tilus.hidet.ir.dialects.pattern import PlaceholderExpr

try:
    from tilus.hidet.ir.task import Task
except ImportError:
    Task = None
...
try:
    from tilus.hidet.ir.cute.algorithm import TiledCopy, TiledMma
    from tilus.hidet.ir.cute.expr import CallOp
    from tilus.hidet.ir.cute.expr import Op as CuteOp
    from tilus.hidet.ir.cute.layout import ComposedTensorLayout, TensorLayout, TiledTensorLayout
    from tilus.hidet.ir.cute.ops import (
        Arithmetic,
        Atomic,
        Broadcast,
        Copy,
    ...
    from tilus.hidet.ir.cute.type import TiledTensorType
except ImportError:
    TiledTensorType = CallOp = CuteOp = TiledTensorLayout = ComposedTensorLayout = TensorLayout = None
    TiledCopy = TiledMma = None

try:
    from tilus.hidet.ir.cute.collective import CollectiveStore
except ImportError:
    CollectiveStore = None
...
class IRPrinter(IRFunctor):
    ...
    def print_tensor_nodes(self, nodes: List[TensorNode], exclude_nodes: List[TensorNode] = None) -> Doc:
        from tilus.hidet.ir.tools import collect  # pylint: disable=import-outside-toplevel
        from tilus.hidet.utils.structure import DirectedGraph
