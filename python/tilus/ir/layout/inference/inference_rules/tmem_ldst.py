...
from tilus.ir.layout.inference.rule import LayoutInferenceError
...
class TMemoryLdstRule(LayoutInferenceRule):
    ...
    @staticmethod
    def inference(ctx: LayoutInferenceContext, inst: TMemoryLoadInst | TMemoryStoreInst) -> dict[RegisterTensor, RegisterLayout]:
        ...
        sliced_tmem_tensor = TMemoryTensor.create(
            dtype=tmem_tensor.dtype,
            shape=regs_tensor.shape,
            first_lane=tmem_tensor.first_lane + inst.offsets[0]
        )
        ...
        if thread_end - thread_begin != lane_end - lane_begin:
            raise LayoutInferenceError(f"The lane range {lane_begin} to {lane_end} does not match the thread range {thread_begin} to {thread_end}")
        ...
        assert layout.shape == regs_tensor.shape, f"Layout shape {layout.shape} does not match tensor shape {regs_tensor.shape} in {inst}"
