...
from tilus.ir.instructions.cuda.tcgen05 import Tcgen05MmaSSInst, Tcgen05MmaTSInst, Tcgen05ScaledMmaSSInst
...
@register_rule(Tcgen05ScaledMmaSSInst)
class Tcgen05ScaledMmaSSRule(LayoutInferenceRule):
    @staticmethod
    def inference(ctx: LayoutInferenceContext, inst: Tcgen05ScaledMmaSSInst) -> dict[SharedTensor, SharedLayout]:
        a_tensor: SharedTensor = inst.inputs[0].as_shared_tensor()
        b_tensor: SharedTensor = inst.inputs[1].as_shared_tensor()
        d_tensor: TMemoryTensor = inst.inputs[2].as_tmemory_tensor()
        ...
        if inst.cta_group == 1:
            if a_shape[1] != b_shape[0] or a_shape[0] != d_shape[0] or b_shape[1] != d_shape[1]:
                raise LayoutInferenceError(
                    f"A, B, and D must have compatible shapes, but got {a_shape}, {b_shape}, and {d_shape}."
                )
        elif inst.cta_group == 2:
            if a_shape[1] != b_shape[0] or a_shape[0] != d_shape[0] or b_shape[1] * 2 != d_shape[1]:
                raise LayoutInferenceError(
                    f"For cta_group=2, shapes must be compatible, but got {a_shape}, {b_shape}, and {d_shape}."
                )
        else:
            raise LayoutInferenceError(f"cta_group must be 1 or 2, but got {inst.cta_group}.")
