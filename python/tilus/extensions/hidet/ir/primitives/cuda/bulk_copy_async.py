...
def resolve_cp_async_bulk_g2s(l2_evict: Optional[str]) -> str:
...
def resolve_cp_async_bulk_s2g(l2_evict: Optional[str], cp_mask: bool) -> str:
    if l2_evict not in [None, "evict_first"]:
        raise ValueError("l2_evict should be None or 'evict_first'")
...
def register_bulk_copy_async():
    ...
    for l2_evict in [None, "evict_first"]:
        func_name = resolve_cp_async_bulk_g2s(l2_evict)
    ...
    for l2_evict in [None, "evict_first"]:
        for cp_mask in [False, True]:
            func_name = resolve_cp_async_bulk_s2g(l2_evict, cp_mask)
...
def cp_async_bulk_g2s(dst: Expr, src: Expr, size: Expr, mbarrier: Expr, l2_evict: Optional[str] = None) -> Expr:
...
def cp_async_bulk_s2g(
    ...
    ret: Expr
        A function call expression.
    """
    func_name = resolve_cp_async_bulk_s2g(l2_evict, byte_mask is not None)
    args = [dst, src, size]
