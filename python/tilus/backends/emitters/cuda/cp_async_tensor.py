...
from hidet.ir.type import DataType, ArrayType
...
@functools.cache
def get_offset_grid_of_swizzled_layout(dtype_nbits: int, shape: tuple[int, ...], swizzle: TensorMapSwizzle) -> Optional[np.ndarray]:
...
class CopyAsyncTensorBaseEmitter(BaseInstEmitter):
    def resolve_global_tensor_info(self, global_tensor: GlobalTensor, offsets: Sequence[Expr], dims: Sequence[int]) -> GlobalTensorInfo:
        ...
        remap = {layout.axes[dim]: offsets[dim] for dim in indexing_dims}
    ...
    def resolve_shared_tensor_info(self, shared_tensor: SharedTensor) -> SharedTensorInfo:
        ...
        offset_grid: np.ndarray = vectorized_evaluate(expr=layout.offset, var2value={axis: grid[i] for i, axis in enumerate(layout.axes)})
        for swizzle in [
            ...
            swizzled_offset_grid = get_offset_grid_of_swizzled_layout(
                dtype_nbits=shared_tensor.dtype.nbits,
    ...
    def declare_host_buffer(self, name: str, dtype: DataType, shape: Sequence[int]) -> Var:
        ...
        return self.host_builder.declare_var(name=name, tp=TensorType(dtype=dtype, shape=shape, layout=strided_layout(shape=shape)))
    ...
    def create_tensor_map(self, global_info: GlobalTensorInfo, shared_info: SharedTensorInfo, dtype: DataType) -> Var:
        ...
        self.host_builder.append(encode_tensor_map(
