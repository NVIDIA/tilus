...
class BulkCopyAsyncGlobalToSharedInstEmitter(BulkCopyAsyncBetweenGlobalShared):
    def emit_cp_async(
        ...
        self.append(mbarrier_expect_tx_cta_shared(mbarrier_addr=barrier_addr, transaction_bytes=size))
...
class CopyAsyncBulkGlobalToClusterSharedEmitter(BulkCopyAsyncBetweenGlobalShared):
    ...
    def emit_cp_async(
        ...
        barrier_local_addr = self.declare_var(name='barrier_addr', tp=uint32, init=cvta_generic_to_shared(inst.mbarrier))
        with self.if_then((1 << self.block_rank_in_cluster) & inst.cta_mask):
            ...
            # ))
            self.append(mbarrier_expect_tx_cta_shared(mbarrier_addr=barrier_local_addr, transaction_bytes=size))
        with self.if_then(self.block_rank_in_cluster == uint32(self.get_smallest_block_rank(inst.cta_mask))):
            ...
            #     self.blockIdx.x, self.blockIdx.y, self.blockIdx.z, self.block_rank_in_cluster, self.current_worker,
            #     shared_address, global_address, int32(size), barrier_local_addr, int32(inst.cta_mask)
            # ))
            self.append(mbarrier_expect_tx_cluster_shared(mbarrier_addr=barrier_local_addr, transaction_bytes=size))
            self.append(cp_async_bulk_global_to_cluster_shared(
                dst=shared_address,
                src=global_address,
                size=int32(size),
                mbarrier=barrier_local_addr,
