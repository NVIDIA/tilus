...
In the next version, we introduce **2-CTA clusters** that enable **distributed
shared memory** and **multicast TMA**, allowing a single TMA load to populate
shared memory across both CTAs simultaneously.
