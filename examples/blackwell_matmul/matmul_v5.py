...
all workers are consumers of the FetchBlockPipeline
"""
...
class FetchBlockPipeline(tilus.Pipeline):
    def __init__(self):
        super().__init__(
            num_stages=2,
            producer_arrive_count=1,
            consumer_arrive_count=4,  # the four workers
        )
        self.thread_blocks = self.shared_tensor(dtype=int32, shape=[self.num_stages, 4])
...
class MmaPipeline(tilus.Pipeline):
    def __init__(self, s: Scheduler, num_stages: int):
        ...
        self.t_acc: list[TMemoryTensor] = [
            self.tcgen05.alloc(float32, shape=[s.block_m, s.block_n]) for _ in range(num_stages)
        ]
