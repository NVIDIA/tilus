...
SM over an internal link (DSMEM). Each CTA only needs to load and store **half
...
- :meth:`tcgen05.mma <tilus.lang.instructions.tcgen05.Tcgen05InstructionGroup.mma>`
  is issued by **a single warp from either CTA** in the pair. The hardware
  reads A from the issuing CTA's shared memory and B from **both CTAs' shared
  memory** (via DSMEM), then writes results to both CTAs' tensor memory. In this
  kernel, we choose CTA 0 as the issuer.
...
Because only CTA 0 issues MMA, the MMA warp on CTA 1 is idle during the
K-loop. CTA 1's MMA warp only participates in consuming CLC responses.
...
**TMA barriers**: CTA 0 owns the ``tma_pipe`` barriers. CTA 1 maps to CTA 0's
barrier address using
...
to declare expected bytes on **both CTAs' barriers** simultaneously. The CLC
response is multicast to both CTAs via
...
with ``target_rank=0`` and ``scope="cluster"``.
...
`examples/blackwell_matmul/matmul_v6.py <https://github.com/NVIDIA/tilus/blob/main/examples/blackwell_matmul/matmul_v6.py>`__.
